...
r.patch('/:id/estado', async (req, res, next) => {
  try {
    ...
    if (estado === 'activo') {
      await query(`UPDATE sprints SET estado='cerrado' WHERE estado='activo' AND id_sprint != ?`, [id])
    }

    await query(`UPDATE sprints SET estado=? WHERE id_sprint=?`, [estado, id])
    const [sp] = await query('SELECT * FROM sprints WHERE id_sprint=?', [id])
    res.json({ success:true, data:sp })
  } catch (err) { next(err) }
})
